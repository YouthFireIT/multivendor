...
class HomeController extends Controller
{
    ...
    //// top ranking part ////
    public function top_ranking_products()
    {
        $allCategory = Category::get();
        return view('frontend.alibaba.topRankingProducts', compact('allCategory'));
    }

    public function fetch_single_category($id)
    {
        $subCategories = SubCategory::where('category_id',$id)->get();
        return view('frontend.alibaba.subCategories', compact('subCategories'));
    }

    //// top selection part ////
    public function top_selected_products()
    {
        $topSelectionProducts = DB::table('products')->select('*')->where('featured', 1)->inRandomOrder()->take(60)->get();
        return view('frontend.alibaba.topSelectionProducts', compact('topSelectionProducts'));
    }

    public function get_categorywise_products($categoryId)
    {
        $products = Product::where('category_id',$categoryId)->where('featured', 1)->inRandomOrder()->take(60)->get();
        return view('frontend.alibaba.categorywiseProducts', compact('products'));
    }

    //// New Arrival Products ////
    public function latest_products()
    {
        $latestProducts = DB::table('products')->select('*')->where('featured', 1)->orderBy('id', 'desc')->get()->take(60);
